...
public class MigrationsExceptionTest
{
    ...
    [Test]
    public async Task Should_Log_DowngradeVersionException_When_Try_To_Upgrade_When_Exist_A_Downgrade_Version_Less_Than_Upgrade_Version()
    {
        var databaseName = Guid.NewGuid().ToString();
        var database = _client.GetDatabase(databaseName);
    ...
    [Test]
    public async Task Should_Log_DowngradeAppliedVersionException_When_Try_To_Downgrade_And_Exist_A_Applied_Version_Greater_Than_Downgrade_Version()
    {
        var databaseName = Guid.NewGuid().ToString();
        var database = _client.GetDatabase(databaseName);
        ...
        logInput[0].Should().Be(LogLevel.Error);
        logInput[2].ToString().Should().Be($"[{databaseName}] Failed to apply migration");
        var expectedMessage = "You have a version 4.0.4 already applied Food, that is greater than you will do a downgrade. Your downgrade FoodPriceSeed - 1.0.4 must respect the order to apply";
        logInput[3].GetType().Should().BeAssignableTo<DowngradeAppliedVersionException>();
        logInput[3].As<DowngradeAppliedVersionException>().Message.Should().Be(expectedMessage);
    }
}